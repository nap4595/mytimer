# MultiTimer v1.2.1 - 프리셋 기반 다중 타이머

프리셋 기반 다중 타이머 웹 애플리케이션 - 5개/10개/15개 타이머를 고정 그리드 레이아웃으로 관리할 수 있는 고성능 웹 앱입니다.

## 🎯 주요 특징

### 💫 **v1.2.1 새로운 기능**
- **프리셋 타이머**: 5개/10개/15개 타이머 프리셋 선택
- **고정 Grid 레이아웃**: CSS Grid 기반 반응형 디자인
- **성능 최적화**: 60fps 배치 렌더링, 메모리 누수 방지
- **접근성 강화**: WCAG 2.1 AA 준수, 스크린 리더 지원
...
- **독립적인 타이머 동작**: 프리셋에 따라 타이머가 서로 영향 없이 동작
...
> 🎯 **성능**: 60fps 보장, WCAG 2.1 AA 준수, 메모리 누수 방지